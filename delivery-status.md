...
| 22 | 研究者可以讓化學狀態調節局部學習與記憶表現、施加具名干預、訓練小型控制器並和簡單替代模型對照 | 主 agent 指揮 / opencode big-pickle 小票實作 | in_progress | 第一階段驗證通過：受體驅動的閘門與時間窗（閘門關只衰退、脈衝開閘、`decay_e` 隨佔用率變化、明確閘門與受體閘門不得並用）、讀出增益只動讀出且清除後逐位恢復（不是遺忘）、穩定化寫入 `slow` 有觸發／去重／預算、三層 L2 分開；MOD-05、MOD-06 passed；第二階段（干預，COR-11）小票進行中；第三階段（控制器與對照）待派工；第二階段進行中（2026-09-19）：個體上的節點類干預 `Intervene`（clamp／force_spike／silence、孿生比較日誌）與原生 runner 的節點類干預都已落地並提交，通道類干預與 COR-11 證據待收；第三階段控制器來源第一次派工偏離契約已丟棄、重派待限流解除 |
...
| 27 | 使用者可以用離線與 HTTP 教師蒸餾學生，並在完全移除教師的情況下評估它 | 主 agent 指揮 / opencode big-pickle 小票實作 | in_progress | 第一階段驗證通過：統一教師契約、JSONL 紀錄檔與測試輸入分離、離線／重播教師零網路（換掉 transport 仍通過）、HTTP 教師的逾時／重試／4xx／限速／預算／去重／允許清單／金鑰只在 header 全在本機 httptest 驗證、封鎖教師計數；TCH-01、TCH-02 passed；真實遠端未驗證；第二階段（蒸餾）與第三階段（移除教師評估、工具安全）待派工；第二階段進行中（2026-09-19）：`distill` 的學生編碼器、訓練／保留劃分與教師收集、分布蒸餾對齊與損失已落地並提交；接梯度入口與證據待 26 的 `LossGradientFrom` |
...
| 26 | 研究者可以量測先學 A 再學 B 的保持與適應、完成專家模仿與行動回饋學習，並執行有來源的生物啟發干預協定 | 主 agent 指揮 / opencode big-pickle 小票實作 | in_progress | 第一階段進行中（2026-09-19）：凍結化學、協定型別、產生器／遺忘／彙總、fixture 與孿生評估、完整矩陣執行（保留所有 seed 與失敗）已落地並提交；狀態切換與 independent 對照、事前比較、CLI 與證據待派。第二階段的 PPO 純數學（GAE、裁切目標與梯度）與 gridnav 環境已落地；第三階段的生物證據登錄已落地 |
