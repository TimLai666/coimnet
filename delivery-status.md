...
| 24 | 使用者可以用嚴格展開的組態、啟動前的資源預估、完整 CLI 與 SDK，並安全遷移格式與保護敏感資料 | 主 agent 指揮 / Opus 5 實作 | in_progress | 第一階段驗證通過：`coimnet-config/v1` 嚴格解碼、四層來源追蹤、金鑰只存參照、名稱註冊表拒絕未實作；資源預估兩個規格算術釘住、超限拒絕不縮減、全圖預估 0.96 GiB 與 NAT-01 實測 RSS 並列；`run --config --dry-run` 零副作用、退出碼 0／2／1；OPS-03、OPS-06 passed；第二、三階段待派工；第三階段驗證通過：`checkpoint.Migrate` 與 CLI `checkpoint migrate` 不覆寫原件、報告前後 SHA-256 與欄位變更（舊形個體 → 聯集形，無資訊損失）、`internal/redact` 遮罩金鑰與原始內容、`scripts/scan-commit.sh` 與 pre-commit 範本、無遙測與 net/http 允許清單測試（internal/cli 的 `data sources` HEAD 檢查為既有偏差）；STA-05、STA-06 passed；第二階段 SDK／CLI 待派工；第二階段進行中（2026-09-19）：SDK 八個可執行範例、87 組非法輸入不 panic、五個取消釋放測試（goroutine 與 HeapInuse 回到基線）、CLI `model inspect`／`model validate` 已提交；`benchmark` 前三階段半成品等限流解除接手，`export`／`report` 與 OPS-01／OPS-02 證據待派工 |
...
| 27 | 使用者可以用離線與 HTTP 教師蒸餾學生，並在完全移除教師的情況下評估它 | 主 agent 指揮 / opencode big-pickle 小票實作 | in_progress | 第一階段驗證通過：統一教師契約、JSONL 紀錄檔與測試輸入分離、離線／重播教師零網路（換掉 transport 仍通過）、HTTP 教師的逾時／重試／4xx／限速／預算／去重／允許清單／金鑰只在 header 全在本機 httptest 驗證、封鎖教師計數；TCH-01、TCH-02 passed；真實遠端未驗證；第二階段（蒸餾）與第三階段（移除教師評估、工具安全）待派工；第二階段進行中（2026-09-19）：`distill` 的學生編碼器、訓練／保留劃分與教師收集、分布蒸餾對齊與損失已落地並提交；接梯度入口與證據待 26 的 `LossGradientFrom`；第二階段（2026-09-19）：蒸餾接上 `StepFrom`（`StepDistribution`／`StepLabel`／`Agreement`，held-out 一致率 0→1，Ratio 為更新前一致率）已提交，TCH-03／TCH-04 證據待派；第三階段學生獨立評估（student 模式）票已寫好待派 |
...
| 29 | 使用者可以用同一核心做 OCR、語音轉文字、文字生成與文字條件影音生成，並把有授權的真實任務資料帶入框架 | 主 agent 指揮 / opencode big-pickle 小票實作 | in_progress | 第一階段進行中（2026-09-19）：`tasks/ocr/ctc`（log-space CTC 與其梯度，對路徑枚舉與有限差分逐位／1e-12 對照，貪婪解碼）、CER／WER（固定回溯順序、空參考不除零）、程式生成字形 fixture（族別分割、雜訊、未見組合）已提交；頁面切行半成品等限流解除接手；每步讀出、單行辨識器、fixture 流程與 CLI 票已寫好；第二階段 WAV／log-mel 與第三階段 tokenizer 票已寫好；真實資料 TSK-11 blocked_data |
