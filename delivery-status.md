...
| 29 | 使用者可以用同一核心做 OCR、語音轉文字、文字生成與文字條件影音生成，並把有授權的真實任務資料帶入框架 | Codex 主 agent / Luna max | in_progress | OCR fixture 已有 TSK-01 證據；ASR 整檔 CTC、因果串流與恢復有前次 Mac／Ubuntu 局部證據，本輪資料匯入、群組分割、離線耗時、CLI 人工範例與 Mac 全套驗證見 TSK-02；嚴格串流檔案保存、TSK-11 真實授權資料與後續生成階段保留 |
...
下一個可驗證成果是 [ticket 29 第二階段](docs/tickets/29-real-tasks-ocr-asr-text-and-media-generation.md) 的嚴格串流快照檔案保存／載入（容量、身分及原子落盤），再以使用者提供的授權自然語音 manifest 執行匯入、訓練、推論與評估。現有三種人工音調不能當成自然語音能力證據。真實全圖／GPU 訓練維持原驗收條件。
