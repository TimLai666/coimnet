...
| 22 | 研究者可以讓受體佔用率調節學習與記憶表現、施加具名干預、訓練小型控制器並對照 | 主 agent 指揮 / opencode big-pickle 小票實作 | in_progress | MOD-06 前半（讀出增益）與後半（`slow` 穩定化寫入）已通過：`Consolidate` 手算、去重／預算／閘門拒絕、slow 折進基礎權重孿生逐位、快照與 checkpoint 往返、接續後去重同連續執行；MOD-05 閘門與時間窗、控制器（MOD-07）、對照流程（MOD-10）、干預（COR-11）與 `evidence/MOD-06/` 待派工或建置 |
