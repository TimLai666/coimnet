...
| 11 | 研究者可執行並訓練 LIF 放電核心 | 主 agent 指揮 / Opus 實作 | in_progress | dynamics LIF 通過手算、tangent 參考、平滑模式有限差分；learning 可選 LIF 核心、theta 參數群組、快照往返；`examples/lifthreshold` 只訓練閾值保留 MSE 0.232→0.052；第三階段 CLI／文件／證據進行中 |
