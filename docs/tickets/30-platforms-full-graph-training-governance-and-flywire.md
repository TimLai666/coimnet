...
## 第四階段效能報告證據（2026-09-24）

`coimnet benchmark` 在固定的 4030b0d 跑了三次：預設合成 fixture 分開跑兩個程序，真實全圖以 store 模式跑一次（8 列、
2 次重複，`GOMEMLIMIT=8GiB`）。六個階段都分開記初始化、傳輸（CPU 為 0）、暖機與穩態，能耗固定寫 `measured: false`。
