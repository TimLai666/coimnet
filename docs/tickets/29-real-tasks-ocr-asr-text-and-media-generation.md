...
第五階段拆成四張小票：影片 fixture 與原生輸出（29E-01）、核心逐格生成與 `RunVideo`（29E-02）、選用封裝器與
`doctor` 探測（29E-03a）、`examples run video` 與證據輔助（29E-03b）。證據 `evidence/TSK-06/`：六個片段的
影格、WAV 與時間線都完整，本機 ffmpeg 8.1.2 六個都封裝成功，缺席工具記成 `tool_absent`；假工具失敗的情況由
測試證明會記成 `tool_failed` 並清掉半成品。同一 commit 重跑，報告、摘要與所有原生檔逐位元組相同；mkv 容器
每次由 ffmpeg 寫入不同的中繼資料，不列入比對。

結果照實寫：預設 80 輪時模型偏弱，完整核心已見 5 個片段對 3 到 4 個、同步對齊 0 到 3 個；多數同步誤差是
-1，因為動點軌跡不夠準，判讀器找不到碰到邊緣的那一格。完整核心把嗶聲放在正確的第 7 格的次數遠多於凍結
核心。事後加跑 160 與 320 輪：160 輪凍結核心領先，320 輪完整核心同步 4 到 5 個、凍結核心 0 到 3 個。比較
隨訓練量翻轉，不宣稱核心學習的必要性，預設也沒有依這些結果回頭調整。

## 第六階段證據（2026-09-24）

第六階段拆成三張小票與一次審查退修：固定還原器與潛在表示生成器（29F-01）、請求頭與白名單外部工具
（29F-02a）、`RunRoles` 角色報告（29F-02b）。證據 `evidence/TSK-07/`：三種模式都學會
8 個已見條件；只有外部工具模式在兩個 seed 答對留出的 blue diagonal，因為請求把顏色和形狀分成兩格，像素由
工具產生，報告把它放在 `tool_result`，核心只算對的請求。固定還原器單獨就能還原 9 張目標，報告以雜湊與參數量
標明它提供的能力。注入永遠回傳 red square 的假工具時，外部工具模式的核心能力與其他模式都不變、只有工具結果
改變，證明工具結果不計入核心。外部工具是本機替身，沒有呼叫第三方模型或網路。
