...
| 可訓練控制器 | 已有 CPU 實作（fixture） | `modulation.Controller` 是一個小 MLP 來源：輸入 = 最近 `SummaryWindow` 步活動的均值與方差摘要 + 已可取得的回饋分數 + 宣告的資源（`ControllerInputs.Resources` 是名稱清單，沒宣告的資源讀 0），輸出 `softplus` 保證非負的釋放率。活動視窗是跨呼叫保留的狀態，**不在任何快照**（復原後視窗從零重來）。梯度只經自身輸出對宣告的控制目標（`reward_proxy`｜`activity_target`，都從 Release 剛讀過的同一份輸入取值）計算，**不穿過濃度→受體→核心**；`ControllerInputs` 沒有 Target 欄位，`scripts/check-target-flow.sh` 涵蓋 `modulation`。參數量與每步乘加經 `ParameterCount()`／`MultAddsPerStep()` 可查並寫進對照報告的 per-group `parameters`。容量匹配對照 `MemoryController` 是同參數量的一般記憶控制器（同 MLP 形狀，線性輸出直接加到讀出而非釋放率）。見 [ticket 22](tickets/22-modulated-learning-memory-controller-and-interventions.md) 與 `evidence/MOD-07/`。 |
| 對照流程 | 已有 CPU 實作（fixture） | `experiment.RunAblation` 與 CLI `examples run ablate` 用同一資料分割、同 seed、同更新預算與同指標跑五組 `no_modulation | direct_reward | fixed_decay | trainable_controller | capacity_matched`，每組獨立存 `ablation/<group>.json`，`summary.json` 列任務指標、參數量與對同 seed no_modulation 保留評估讀出的 L2 差（activity_delta）。`direct_reward` 把前一 episode 的獎勵直接當學習閘門，是「不得僅將 reward 改名為 hormone」的對照；`fixed_decay` 的濃度由固定 `ExternalTimeline` 給、不受學習影響；`trainable_controller` 的釋放經 `internal_resource` 進化學層，控制器只看活動摘要與已到達的回饋、永遠看不到 target；`capacity_matched` 的位移只進該組指標、不進基礎參數的梯度。報告記錄數字不做文字判斷。見 [ticket 22](tickets/22-modulated-learning-memory-controller-and-interventions.md) 與 `evidence/MOD-10/`。 |
| 適應性評估 | 已有 CPU 實作 | 基礎參數凍結、適應分割允許宣告的局部更新、評分分割只推論、題間依政策重設、三項污染檢查，證據 `evidence/LRN-10/`。 |
| 教師輔助評估（teacher_assisted） | 已有 CPU 實作（fixture） | `experiment/studenteval` 除 `student` 外的第二種模式：評估期每筆保留集與獨立集輸入各問一次教師，報告的 `mode` 欄位標 `teacher_assisted`、`TeacherBlocked` 為 false，結果檔與 `student` 模式分開。**它的分數含教師答案，不是學生能力**：assisted 分數把教師答案（教師答不出來時退回學生 argmax 並計 `fallbacks`）對真實標籤計分，報告的 `assumptions` 附加「teacher_assisted scores include the teacher's answers; they are never reported as the student's own capability.」；學生自己的分數在兩種模式下逐位相同。見 [ticket 27](tickets/27-teachers-and-distillation.md) 與 `evidence/TCH-05/`。 |
| 持續學習矩陣 | 已有 CPU 實作（fixture） | 協定 `experiment.ContinualProtocol`（有版本的 `task` 與 generator、`train_task`／`rule_change` 階段、至少 3 個 seed、評估與事前比較設定）跑「每階段結束對所有任務評估」的完整矩陣 `R[i][j]`。分數是 `neg_mse`（越大越好），評估在快照孿生上做，孿生先凍結化學濃度再重設神經狀態，結束時 Parameters 與 Plastic 必須逐位不變，所以評分不推進個體。遺忘 `F_j(i) = max_{k ≤ i, 未失敗} R[k][j] − R[i][j]`，失敗格 F=0。`rule_change` 把該任務的目標 gain 取負，之後該任務的訓練與評估都用新規則。**失敗執行與 seed 全部保留**：每個失敗的 stage／評分／控制都是一條 `RunRecord`，聚合 `cells` 只用成功 seed 並標 `succeeded/total`。`fixed_chemistry` 與 `state_switch` 把「暫時抑制」與「遺忘」分開（相同個體在另一組化學濃度下重評，分數變而參數與塑性逐位不變，是表現不是遺忘）。`independent` 是每個任務重新初始化的對照，依協定順序只重放該任務自己的 stage，報告分開列、不折入矩陣平均。比較只在協定事前宣告 `paired_bootstrap` 時附上，區間是 bootstrap 百分位區間不是顯著性宣稱；未宣告的報告不做「勝過」陳述。CLI `examples run continual-matrix`（`--chemistry` 開啟凍結與狀態切換、`--out` 不覆寫）。見 [ticket 26](tickets/26-continual-matrix-imitation-ppo-and-bio-inspired-protocols.md) 與 `evidence/LRN-08/`。
| 暫時連線狀態、局部學習、近期參與紀錄 | 待實作 | 分開保存基礎參數及暫時狀態，驗證更新次序、關閉效果與恢復。 |
| 人工調節與有證據支持的生物調節 | 已有 CPU 實作（fixture，兩種假說協定） | 人工設定使用功能名稱。生物設定附來源、單位、接收端與未知欄位，不能只改名冒充荷爾蒙。目前落地的是兩個登錄過來源的假說協定（`experiment.RunBioInspired`）：`ecdysone_inspired`（登錄 S14，在同一個學習 episode 內移動 `ExternalTimeline` 脈衝的時機，量測慢速層 L2 幅度與重測分數隨時機變化的曲線）與 `npf_memory_expression_hypothesis`（登錄 S15，受體驅動的讀出增益抑制後再切回中性重評，分數下降又回到與抑制前逐位相同，而 `Parameters`、`slow`、`plastic` 三層逐位不變，所以是表現不是遺忘）。**協定名稱只有這兩個**：要標成 `ecdysone`／`npf` 定量模型必須 `QuantitativeEvidence` 的 Method、Data、Fit、HeldOutIntervention 四項齊全，缺項回 `ErrQuantitativeNaming`，四項補齊也只是通過驗證而不會跑出數字（定量協定未實作）；報告的 `assumptions` 明寫這些是允許假設名稱下的工程數值，不是定量生物重現。見 [ticket 26](tickets/26-continual-matrix-imitation-ppo-and-bio-inspired-protocols.md) 與 `evidence/MOD-09/`。 |
