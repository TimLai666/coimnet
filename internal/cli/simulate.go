...
func runSimulateRun(ctx context.Context, args []string, stdout, stderr io.Writer) error {
	...
	// The state snapshot is the core's state and carries no fast changes, so a
	// plastic run split across two processes would silently restart them at
	// zero. Refuse the combination instead of dropping state quietly.
	if protocol.Plasticity != nil && (stateIn != "" || stateOut != "") {
		return fmt.Errorf("--state-in and --state-out carry the core state only; this protocol declares a plasticity block, whose fast changes the snapshot cannot hold, so a run split across processes would silently restart them at zero. Run the protocol in one pass, or remove the block")
	}
