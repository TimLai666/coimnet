...
Commands:
  ...
  examples run media [flags]    Run text-conditioned synthetic image/audio
                                generation with a trained core and fixed decoder
  examples run roles [flags]    Compare core, fixed-decoder and external-tool
                                image generation roles
  examples run video [flags]    Generate prompt-conditioned video frames and
                                aligned audio, with optional packaging
	...
	case "examples":
		...
		if len(args) == 2 && args[1] == "list" {
			return writeJSON(stdout, []map[string]string{
				...
				{"name": "media", "profile": "fixture", "description": "Synthetic text-conditioned image and audio generation with a trained core, fixed decoder and held-out conditions"},
				{"name": "roles", "profile": "fixture", "description": "Compare core-generated pixels, a frozen decoder and an external tool on synthetic image prompts"},
				{"name": "video", "profile": "fixture", "description": "Synthetic prompt-conditioned video and aligned audio generated by a trained core with optional video packaging"},
		...
		if len(args) >= 3 && args[1] == "run" && args[2] == "media" {
			return runMedia(ctx, args[3:], stdout, stderr)
		}
		if len(args) >= 3 && args[1] == "run" && args[2] == "roles" {
			return runRoles(ctx, args[3:], stdout, stderr)
		}
		if len(args) >= 3 && args[1] == "run" && args[2] == "video" {
			return runVideo(ctx, args[3:], stdout, stderr)
		}
