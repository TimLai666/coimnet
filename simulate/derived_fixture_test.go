...
// The fixture below is the hand-calculated derivation fixture of ticket 13,
// rebuilt here because Go cannot import another package's test files. Only the
// writers and the row tables are copied; every expected value used by the
// tests in this package is written out again from the ticket table, not read
// back from the params package.
//
...
// derivedFileSum returns the SHA-256 of a file in dir.
...
// derivedGraphFixture writes the three connectome sources into dir and builds
// the four-node, six-edge graph the derivation fixture is calculated for.
func derivedGraphFixture(t *testing.T, dir string) *connectome.Graph {
	...
	manifest := connectome.DatasetManifest{
		...
		Namespace:     "simulate-derived-fixture-v1",
		SourceVersion: "v1",
...
// float64 averaging are both exact.
...
var derivedTbarRows = []derivedTbarRow{
	{x: 1, y: 2, z: 3, body: 10, probs: p16(12, 1, 1, 1, 1, 0, 0), nullProb: -1},    // acetylcholine 0.75
	{x: 4, y: 5, z: 6, body: 10, probs: p16(8, 2, 3, 1, 1, 1, 0), nullProb: -1},     // acetylcholine 0.5
	{x: 7, y: 8, z: 9, body: 10, probs: p16(2, 1, 11, 1, 1, 0, 0), nullProb: -1},    // gaba 0.6875
	{x: -1, y: -2, z: -3, body: 20, probs: p16(3, 1, 2, 9, 1, 0, 0), nullProb: -1},  // glutamate 0.5625
	{x: 10, y: 11, z: 12, body: 30, probs: p16(0, 1, 1, 1, 1, 0, 0), nullProb: 0},   // null probability
	{x: 13, y: 14, z: 15, body: 10, probs: p16(14, 1, 1, 0, 0, 0, 0), nullProb: -1}, // duplicate key
	{x: 13, y: 14, z: 15, body: 10, probs: p16(1, 1, 14, 0, 0, 0, 0), nullProb: -1}, // duplicate key
	{x: 16, y: 17, z: 18, body: 40, probs: p16(8, 2, 2, 2, 1, 1, 0), nullProb: -1},  // no exact synapse
	{x: 20, y: 21, z: 22, body: 10, probs: p16(3, 10, 1, 1, 1, 0, 0), nullProb: -1}, // dopamine 0.625
	...
	{x: 2, y: 2, z: 2, body: 20, probs: p16(2, 1, 11, 1, 1, 0, 0), nullProb: -1}, // gaba, pair not in graph
}
...
func derivedBodyStatsSchema() *arrow.Schema {
...
func writeDerivedBodyStats(t *testing.T, path string, rows []derivedBodyStatRow) {
	t.Helper()
	writeFeather(t, path, derivedBodyStatsSchema(), func(b *array.RecordBuilder) {
...
func derivedTbarSchema() *arrow.Schema {
	fields := []arrow.Field{
...
func writeDerivedTbar(t *testing.T, path string, rows []derivedTbarRow) {
	t.Helper()
	writeFeather(t, path, derivedTbarSchema(), func(b *array.RecordBuilder) {
...
func derivedSynPartnersSchema() *arrow.Schema {
	return arrow.NewSchema([]arrow.Field{
...
func writeDerivedSynPartners(t *testing.T, path string, rows []derivedSynRow) {
	t.Helper()
	writeFeather(t, path, derivedSynPartnersSchema(), func(b *array.RecordBuilder) {
		x := b.Field(0).(*array.Int32Builder)
		y := b.Field(1).(*array.Int32Builder)
		z := b.Field(2).(*array.Int32Builder)
...
// derivedMetaCSV is a one-row Neuprint_Meta.csv in the official neo4j import
// shape: header names carry a ":type" suffix and roiHierarchy is JSON.
...
func derivedRules(t *testing.T, dir string) params.Rules {
	t.Helper()
	writeDerivedBodyStats(t, filepath.Join(dir, "body-stats.feather"), derivedBodyStatRows)
	writeDerivedTbar(t, filepath.Join(dir, "tbar.feather"), derivedTbarRows)
	writeDerivedSynPartners(t, filepath.Join(dir, "syn-partners.feather"), derivedSynRows)
...
// derivedFixtureSet runs the real derivation over the fixture and returns the
// parameter set together with the graph it was derived for.
func derivedFixtureSet(t *testing.T) (*params.Set, *connectome.Graph) {
	t.Helper()
	dir := t.TempDir()
