...
- 2026-09-24 GPU 後端選 WebGPU 作為跨 Mac Metal、Linux Vulkan 與 Windows 的第一條裝置路徑；CPU float64 仍是數值參考。2026-09-25 已在 Mac Metal 驗證 float32 稀疏 drive 與局部反向導數，並以純量零延遲連續核心 adapter 在逐步前向／反向和 CPU 對照。前向拓樸依 target、反向依 source 分組，保留各組內邊宣告順序；漏寫梯度的 Metal shader 迴圈改為每邊獨立寫入並連續重跑驗證。可選的 `learning.NewGPUTrainer`／`RestoreGPUTrainer` 已讓獨立 episode 的 `Step`／`StepFrom` 使用這條核心，並通過 CPU 參數／AdamW 數值對照與 JSON 快照新程序續訓。編碼器與讀出仍走既有 Insyra 路徑，是否由 Insyra 在裝置執行未量測；神經狀態、活化與 AdamW 在 CPU，沒有裝置上參數更新或常駐神經狀態。LIF、混合、延遲、向量／矩陣、重算與完整圖均未支援／驗證，因此 OPS-05 仍未通過。裝置選取、實際執行與回退須分別報告；Insyra dense MatMul 不算稀疏核心加速。Insyra v0.3.2 缺稀疏裝置運算及裝置狀態公開契約，追蹤 [#379](https://github.com/HazelnutParadise/insyra/issues/379)。
...
實測資源與適用設定見 [資源說明](docs/resources.md)。反向視窗（`Options.Truncation`）只控制梯度傳遞，前向 trace 仍保存完整輸入序列歷史，縮短視窗不會降低歷史配置量；每一步的梯度實際往回追溯幾步寫在 `StepResult.GradientHorizonSteps`（視窗短於輸入列數時是視窗，否則是整段），更早的列仍影響預測與狀態，但收不到梯度。要降低反向歷史用 `Options.Recompute{SegmentSteps}`（LRN-02）：`Step`／`StepFrom` 不保存核心 trace，前向逐段推進，反向從每段起點的檢查點重跑該段，梯度、參數、最佳化器狀態與更新次數和完整歷史逐位相同；`Predict`、`PredictAll`、`Spikes` 一律走完整前向。只有連續與 LIF 核心支援，混合與向量核心在 `NewTrainer`、`RestoreTrainer`、`Individual.ResetOptimizer` 寫入前就拒絕。重算省下的是核心歷史（`T = 256`、`S = 16` 時從 257 列降到 81 列），網路層仍保留整段的核心輸入、核心輸出、上游梯度與輸入梯度，所以實測最大 RSS 只省 24–34%，不是數量級；重算經串流狀態推進，節點數 ×（最大延遲 + 1）不得超過 `dynamics.MaxStateValues`。訓練回合從零狀態開始、不碰可塑性、化學狀態與亂數，所以重算不會重複學習、重複隨機事件或呼叫教師，證據見 [LRN-02](evidence/LRN-02/verification.json)。啟動前記憶體預估（OPS-06）已由 `resources.Estimate` 提供；全圖訓練（OPS-07）依原需求追蹤，文件估算不取代這項功能。
