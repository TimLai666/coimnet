...
// DefaultOptions returns all groups trainable and a declared AdamW baseline.
func DefaultOptions() Options {
	return Options{.01, .9, .999, 1e-8, 0, 1, 0, Trainable{true, true, true, true, true}}
}

// AdamState is ordered weights, bias, log_tau, encoder, readout. Each parameter
// counts its own unfrozen updates, preserving bias correction across freezes.
...
func copyParameters(p Parameters) Parameters {
	return Parameters{Core: dynamics.Parameters{Weights: append([]float64(nil), p.Core.Weights...), Bias: append([]float64(nil), p.Core.Bias...), LogTau: append([]float64(nil), p.Core.LogTau...)}, Encoder: append([]float64(nil), p.Encoder...), Readout: append([]float64(nil), p.Readout...)}
}
...
func flatParameters(p Parameters) []float64 {
	out := []float64{}
	for _, v := range [][]float64{p.Core.Weights, p.Core.Bias, p.Core.LogTau, p.Encoder, p.Readout} {
		out = append(out, v...)
	}
	return out
}
func flatGradient(g Gradient) []float64 {
	out := []float64{}
	for _, v := range [][]float64{g.Core.Weights, g.Core.Bias, g.Core.LogTau, g.Encoder, g.Readout} {
		out = append(out, v...)
	}
	return out
}
func unflatten(v []float64, shape Parameters) Parameters {
	...
	offset := 0
	for _, dst := range [][]float64{p.Core.Weights, p.Core.Bias, p.Core.LogTau, p.Encoder, p.Readout} {
...
func parameterMask(p Parameters, m Trainable) []bool {
	...
	for _, g := range []struct {
		size    int
		enabled bool
	}{{len(p.Core.Weights), m.Weights}, {len(p.Core.Bias), m.Bias}, {len(p.Core.LogTau), m.Tau}, {len(p.Encoder), m.Encoder}, {len(p.Readout), m.Readout}} {
		for range g.size {
