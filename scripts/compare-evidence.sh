...
if [[ $# -lt 2 || $# -gt 3 || $1 == --help || $1 == -h ]]; then
  echo 'Usage: scripts/compare-evidence.sh NEW_OUTPUT_DIRECTORY PROTOCOL_FILE [PARAMS_FILE]'
  ...
  echo 'one run report per cell under cells/. The matrix runs the original wiring and'
  echo 'every declared null model seed, so it takes as many whole-brain runs as the'
  echo 'protocol declares cells; no seed, step or limit is lowered here.'
...
printf 'scripts/compare-evidence.sh %q %q %q\n' "$1" "$protocol" "$parameters" > "$output/command.txt"
...
echo 'RESULT: compare evidence recorded'
