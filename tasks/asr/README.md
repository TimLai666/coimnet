...
`stream.Save(ctx, "stream.json")` 把整條串流寫成單一檔案；`r.LoadStream(ctx, "stream.json")` 以相符的辨識器讀回並接續。檔案同時含個體快照、待處理音訊、CTC 狀態、文字、取樣率、前處理版本與字母表。載入時檢查欄位、schema、SHA-256、神經狀態與辨識器語意；它不是單獨的個體快照。保存與讀取的檔案上限是 64 MiB，超過時回錯，不能拿來保存超過此限的完整模型。保存採暫存檔與排他發布，不覆寫既有路徑；取消時未發布的暫存檔會清理。

SHA-256 用來發現損壞，不是簽章。檔案路徑、存取權限及可信來源仍由呼叫端管理；若需保護檔案免於持有寫入權限者竄改，須另加驗證簽章或存放於受控環境。`StreamState` 仍可供記憶體中的 JSON 往返，但自行解碼的 JSON 不等於經 `LoadStream` 嚴格驗證的檔案。
...
測試涵蓋人工音調序列的訓練與保留集 CER／WER、相同設定重現、任意分塊、短尾、取消、快照往返與跨程序檔案恢復、資料清單匯入、說話者／場次分割、逐塊耗時報告與錯誤設定拒絕。端到端測試以人工 WAV 實際走過匯入、分割、訓練及兩種評估。
人工資料只有 `a`、`b`、`c` 對應的音調，不是自然語音資料。沒有自然語音辨識率、時間對齊精度或全腦模型訓練成績。
完整語音需求與尚待完成的真實資料驗收，見 [ticket 29](../../docs/tickets/29-real-tasks-ocr-asr-text-and-media-generation.md)。
