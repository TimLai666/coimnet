...
func TestRunAdaptiveEvaluationFixedMode(t *testing.T) {
	...
	// (c) Adaptive mode returns "not implemented yet" error.
	t.Run("adaptive mode not implemented", func(t *testing.T) {
		ind := newAdaptiveIndividual(t)
		e := experiment.AdaptiveEvaluation{
			Mode: experiment.EvaluationModeAdaptive,
			Adaptation: []experiment.EvaluationItem{
				{ID: "adapt-a", Input: [][]float64{{1, 0}}, Target: []float64{.5}},
			},
			Scoring: []experiment.EvaluationItem{
				{ID: "score-a", Input: [][]float64{{0, 1}}, Target: []float64{-.5}},
			},
			Reset: experiment.ResetPolicy{NeuralAtItemStart: true},
		}
		_, err := experiment.RunAdaptiveEvaluation(ctx, ind, e)
		if err == nil {
			t.Fatal("expected error for adaptive mode")
		}
		if !strings.Contains(err.Error(), "not implemented") {
			t.Fatalf("error %q does not contain %q", err, "not implemented")
		}
	})

	// (d) nil individual returns error.
	...
	// (e) target length differs from output width → error.
